...
class LandingPage(BasePage):
    ...
    def should_be_zip_code_field(self):
        assert self.is_element_present(*LandingPageLocators.FIRST_ZIP_CODE_FIELD), "First zip code field is not present"

    def should_be_get_estimate_button(self):
        assert self.is_element_present(*LandingPageLocators.FIRST_GET_ESTIMATE_BUTTON), "First Get Estimate Button is not present"
    ...
    def click_get_estimate_button(self):
        get_estimate_button = self.browser.find_element(*LandingPageLocators.FIRST_GET_ESTIMATE_BUTTON)
